...
CObjModelLoader ObjModelLoader;
...
bool CObjModelLoader::ReadModelFromFile(string filename) // should open the model with filename specified and process it and then close the file
{
...
void CObjModelLoader::LoadModel(CCalModel& model) // copy the model to the CCalModel object
{
	if(m_pModel)
		model = *m_pModel;
}
...
bool CObjModelLoader::ReadDataFromFile(string filename) // read the data into m_pFileData
{
...
void CObjModelLoader::FreeMemory() // free memory and set to NULL
{
	if(m_pFileData)
		delete [] m_pFileData;
	m_pFileData = NULL;
