...
static unsigned long inw_DIKToINKEY[INW_NUMDIKEYS];

// current and previous key states
static unsigned char keyStates[INKEY_NUMKEYS];
static unsigned char oldKeyStates[INKEY_NUMKEYS];
...
static long keyMouseDragPositions[INKEY_NUMKEYS][2]; // x/y

// mouse limits, set to "no limit" extremes by default
//static sdword mouseMinX=0x80000000, mouseMinY=0x80000000; // signed minimum
//static sdword mouseMaxX=0x7fffffff, mouseMaxY=0x7fffffff; // signed maximum
// current mouse positions
static long mouseX, mouseY; // within limits
static long mouseFreeX, mouseFreeY; // without limits
...
static char mouseExclusive = 1;

// device acquired status
static char mouseAcquired, keyboardAcquired;

// keyboard event injection buffer
static unsigned long keyInjectionBuffer[INW_MAXINJECT][2]; // key/state
static unsigned long keyInjectionBufferIndex;
...
static void CheckKeyEvents(unsigned long key, in_Event *event);

// event index
static unsigned long inw_eventIndex;
...
static void FlushKeyboardData()
{
	...
	memset(keyStates, 0, sizeof(byte)*INKEY_NUMKEYS);
	memset(oldKeyStates, 0, sizeof(byte)*INKEY_NUMKEYS);
...
static void InitDIKToINKEY()
{
	...
	memset(inw_DIKToINKEY, 0, INW_NUMDIKEYS*sizeof(unsigned long));
...
void INW_Update()
{
	...
	in_Event event;
	bbool ready;
	static unsigned long keyForMouseButton[3] = { INKEY_MOUSELEFT, INKEY_MOUSERIGHT, INKEY_MOUSEMIDDLE };
	static bbool keyEventSent[INKEY_NUMKEYS];
	...
	if(inw_lpdi_mouse)
	{
		...
		if (ready)
		{
			...
			for(i=0; i<3; i++)
			{
				...
				keyStates[key] = mstate.rgbButtons[i] & (byte)0x80;
	...
	if (ready)
	{
		...
		for (i=0;i<num;i++)
		{
			...
			// set current key state
			keyStates[key] = (byte)keys[i].dwData & 0x80L;
