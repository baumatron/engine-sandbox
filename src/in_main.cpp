...
{
	unsigned char (*handler)(in_Event *);
...
// shifted key table
static unsigned long in_Shifted[256];
static unsigned long in_Unshifted[256];
...
unsigned long IN_AddReceiver(unsigned char (*handler)(in_Event *))
{
	...
	for (i=0;i<IN_MAXRECEIVERS;i++)
	{
		...
		if (!rec->handler)
		{
			...
			rec->prev->next = rec;
			rec->next->prev = rec;
			return((unsigned long)(rec - in_ReceiverPool) + 1);
		}
	}
	return(0);
}
...
void IN_ChangeReceiverHandler(unsigned long receiver, unsigned char (*handler)(in_Event *))
{
	in_receiver_t *rec;
...
unsigned long IN_KeyForName(char* name)
{
	...
	{
		cout << "Warning: IN_KeyForName is broken! Come fix it please!\n";
